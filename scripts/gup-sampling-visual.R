...
#########################
# MAIN GRAPH
 g<-df%>% ggplot(.,aes(x=x,y=y,group=group,size=pts))+
    geom_point(show.legend = F)+
    scale_y_reverse(expand=expansion(mult=.1))+#expand=expansion(0)
    scale_x_continuous(expand=expansion(mult=.1))+
    scale_size_continuous(range=range(df$pts))+
    labs(x="",y="",title='Count: {format(as.numeric(closest_state),big.mark=",")}')+
    #geom_text(aes(x=labX,y=labY,label=label,vjust="inward"))+
    # geom_text(aes(x=labX,y=labYoffset,label=label,vjust="inward"),color="white")+
    theme_void()+
    theme(plot.margin=margin(0,10,10,10,unit="pt"),plot.title=element_text(family="Montserrat",size=30,hjust = 0.5))+
    gganimate::transition_states(N,state_length=state_len,transition_length=1)+
